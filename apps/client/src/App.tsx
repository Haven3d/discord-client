...
const App: React.FC = () => {
  ...
  const { activeStreamers, participants } = useWebCodecs();
  ...
  // Combine streamers into participants list
  const allParticipants = [...(participants || [])];
  allStreamers.forEach((streamerId: string) => {
    if (!allParticipants.find(p => p.socketId === streamerId || p.user?.id === streamerId)) {
      allParticipants.push({ socketId: streamerId, user: { id: streamerId, username: streamerId === 'dev-user' ? 'Dev User' : 'Usuário' } });
    }
  });
    ...
    <div id="app">
      ...
      {allParticipants.length > 0 ? (
        <main id="grid" className="grid palco" style={{ flex: 1, minHeight: 0, position: 'relative' }}>
            ...
            {watchingStreamerId ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '10px' }}>
                ...
                <p className="muted" style={{ margin: 0, fontSize: '14px', color: 'var(--muted)' }}>
                  {allStreamers.length > 1 ? `${allStreamers.length} pessoas estão transmitindo` : 'Alguém está transmitindo'}
                </p>
              </div>
            ) : (
              <div id="empty" className="empty" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
          ...
          <aside className="sidebar" style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '16px', overflowY: 'auto' }}>
            ...
            {allParticipants.map(p => {
              const id = p.socketId;
              const isUserStreaming = allStreamers.includes(id);
              const userName = p.user?.username || (id === 'dev-user' ? 'Dev User' : 'Usuário');
              ...
              <div key={id} className="tile" style={{ padding: '12px', display: 'flex', alignItems: 'center', gap: '10px', background: 'var(--tile)', borderRadius: '8px', marginTop: '10px' }}>
                ...
                {isUserStreaming && watchingStreamerId !== id && (
        ...
        <div id="empty" className="empty">
          <div className="icon" style={{ fontSize: '48px', opacity: 0.5 }}>👀</div>
          <p id="emptyText" className="muted" style={{ marginTop: '12px' }}>Nenhum participante na call.</p>
        </div>
      )}
